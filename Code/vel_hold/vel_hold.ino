...
void loop() {
  ...
  if (currentMillis - previousMillis >= 10) {  // start timed event
    ...
    if (mydata_remote.toggleTop == 0) {
        ...
        RFB = RFB / 30;  // convert to degrees
        RFB = constrain(RFB,-10,10);
        LT = LT / 100;    // convert to turns
        
        RFBdiff = (RFB/-2) - Output1;
        RFBaccum = RFBaccum + (RFBdiff/50);
        RFBaccumCons = constrain(RFBaccum,-3,3);   
    
        Input1 = pitch;        
        Setpoint1 = pot1 + RFB + RFBaccumCons;
    ...
    Serial.print(RFB);
    Serial.print(" , ");
    Serial.print(RFBdiff);
    Serial.print(" , ");
    Serial.print(RFBaccum);
    Serial.print(" , ");
    Serial.print(RFBaccumCons);
